...
#define TRACE_XCOMPRESSCU 1 // When ebabled: print all calls to xCompressCU, including the block dimension, position, current split series, and encoding modes to be tested

#define TRACE_CTU_COSTS 1 // Trace the best RDCost for each CTU. Works independently from TRACE_XCOMPRESSCU

#define TRACE_CU_COSTS 0 // Trace the best RDCost for each CU. Only works when TRACE_XCOMPRESSCU is enabled

#define TRACE_ENC_MODES 1 // When enabled: print all encoding modes tested by the encoder in each CU (INTRA, INTER, MERGE, SPLITs, ...). Only works when TRACE_XCOMPRESSCU is enabled
